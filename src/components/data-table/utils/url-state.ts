// ** import core packages - TanStack Router only
...
// ✅ Fixed TypeScript types for refs
...
let batchTimeoutId: TimeoutId | null = null;
...
/**
 * Custom hook for managing URL-based state with TanStack Router
 * Provides a simpler approach for storing state in URL params
 */
export function useUrlState<T>(
  ...
  const getValueFromUrl = useCallback(() => {
    ...
    // ✅ TanStack Router: Access search params via search object
    const paramValue = (search as any)?.[key] || search?.[key];
    ...
    if (key === "search" && typeof defaultValue === "string") {
      return decodeURIComponent(paramValue as string) as unknown as T;
    }

    return deserialize(paramValue as string);
  }, [search, key, deserialize, defaultValue]);

  // State to store the current value
  const [value, setValue] = useState<T>(getValueFromUrl());

  // Track the previous search to avoid unnecessary updates
  const prevSearchRef = useRef<any>(null);
  ...
  useEffect(() => {
    ...
    if (
      ...
      JSON.stringify(prevSearchRef.current) === searchString
    ) {
      return;
    }
  ...
  }, [search, getValueFromUrl, key, areEqual]);

  // Synchronously update URL now instead of waiting
  const updateUrlNow = useCallback(
    (newSearch: Record<string, any>) => {
      const now = Date.now();
      lastUrlUpdate.timestamp = now;
      lastUrlUpdate.params = new URLSearchParams();

      // Update the URL immediately with TanStack Router
      router.navigate({
        to: pathname,
        search: (prev: any) => ({ ...prev, ...newSearch }),
        replace: true,
      });
    ...
    (newValue: T | ((prevValue: T) => T)) => {
      ...
      return new Promise<Record<string, any>>((resolve) => {
        const processBatch = () => {
          ...
          // Start with current search as base
          ...
          // Check if sortBy/sortOrder are already in search or in this batch
          const sortByInSearch = currentSearch.sortBy !== undefined;
          ...
          // Build new search object
          const newSearch: Record<string, any> = { ...currentSearch };

          // Iterate over all pending updates and apply them
          ...
          updateUrlNow(newSearch).then(resolve);
        };
