...
import NoDataBlock from "../blocks/no-data";
...
interface DataTableProps<TData extends ExportableData, TValue> {
  ...
  tableNextChildren?: React.ReactNode;
}
...
export function DataTable<TData extends ExportableData, TValue>({
  ...
  if (isError) {
    return (
      <NoDataBlock fullPage className="mt-2" />
    );
  }

  return (
    <div className="mt-2 bg-white font-satoshi-regular p-2 border">
      ...
      <ScrollArea className="rounded-md border my-2">
        <div
        ...
        >
          <Table
            className={cn(tableConfig.enableColumnResizing ? "resizable-table" : "")}
          >
            <TableHeader>
              {table.getHeaderGroups().map((headerGroup) => (
                <TableRow
                  key={headerGroup.id}
                  className="bg-muted hover:bg-muted"  // Add black background
                >
      ...
      {tableNextChildren && tableNextChildren}
